...
import BasePage, { metadata as baseMetadata } from "@/app/cookie/page";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const alternates = buildAlternates("/cookie", locale);
...
export default async function LocaleCookiePage({ params }: PageProps) {
  ...
  return (
    <>
      {!isTranslated && (
        <LocaleFallbackBanner locale={locale} currentPath={`/${locale}/cookie`} />
      )}
      <BasePage />
    </>
  );
