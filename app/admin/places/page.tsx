    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Edit place content and tour metadata. Draft items should not be shown on the public site.
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Link
          ...
          className={`px-3 py-1.5 rounded border text-sm ${
            status === "all" ? "bg-muted font-semibold" : "hover:bg-muted"
          }`}
        >
          All
        </Link>
        <Link
          ...
          className={`px-3 py-1.5 rounded border text-sm ${
            status === "draft" ? "bg-muted font-semibold" : "hover:bg-muted"
          }`}
        >
          Draft
        </Link>
        <Link
          ...
          className={`px-3 py-1.5 rounded border text-sm ${
            status === "published" ? "bg-muted font-semibold" : "hover:bg-muted"
          }`}
        >
          Published
        </Link>
      ...
      <form className="flex gap-2" action={baseHref} method="get">
        ...
        <input
          ...
          placeholder="Search name, slug, country, region…"
        />
      ...
      <div className="border rounded-lg overflow-hidden">
        <div className="grid grid-cols-12 gap-2 px-4 py-2 bg-muted text-xs font-semibold">
          ...
          <div className="col-span-1 text-center">Tour</div>
          ...
          items.map((p) => (
            <div key={p.id} className="grid grid-cols-12 gap-2 px-4 py-3 border-t items-center text-sm">
              <div className="col-span-4">
                <Link className="underline" href={`/admin/places/${p.id}`}>
                  {p.name}
                </Link>
                {p.region ? <div className="text-xs text-muted-foreground mt-1">{p.region}</div> : null}
              </div>

              <div className="col-span-3 text-muted-foreground">{p.slug}</div>
              <div className="col-span-2 text-muted-foreground">{p.country || "-"}</div>
              <div className="col-span-1 text-center">{p.tourHighlight ? "✓" : ""}</div>
              <div className="col-span-1 text-right">{p.tourPriority}</div>

              <div className="col-span-1 text-center">
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded text-xs border ${
                    p.status === "published" ? "bg-muted" : ""
                  }`}
                  title={
